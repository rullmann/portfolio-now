//! Comdirect PDF Parser
//!
//! Parses broker statements from Comdirect.

use super::{
    parse_german_date, parse_german_decimal, BankParser, ParseContext,
    ParsedTransaction, ParsedTransactionType,
};
use regex::Regex;

pub struct ComdirectParser {
    detect_patterns: Vec<&'static str>,
}

impl ComdirectParser {
    pub fn new() -> Self {
        Self {
            detect_patterns: vec![
                "comdirect",
    ...
    fn parse_buy_sell(&self, content: &str, ctx: &mut ParseContext) -> Vec<ParsedTransaction> {
        ...
        // Comdirect patterns
        let is_buy = content.contains("Wertpapierkauf") || content.contains("Kauf von");
        let is_sell = content.contains("Wertpapierverkauf") || content.contains("Verkauf von");

        if !is_buy && !is_sell {
            return transactions;
        }

        let date_re = Regex::new(r"Gesch(?:ä|a)ftstag\s*:\s*(\d{2}\.\d{2}\.\d{4})").ok();
        let isin_re = Regex::new(r"ISIN\s*:\s*([A-Z]{2}[A-Z0-9]{10})").ok();
        let wkn_re = Regex::new(r"WKN\s*:\s*([A-Z0-9]{6})").ok();
        let shares_re = Regex::new(r"St(?:ü|u)ck\s*:\s*([\d.,]+)").ok();
        let price_re = Regex::new(r"Kurs\s*:\s*([\d.,]+)\s*EUR").ok();
        let amount_re = Regex::new(r"Kurswert\s*:\s*EUR\s*([\d.,]+)").ok();
        let provision_re = Regex::new(r"Provision\s*:\s*EUR\s*([\d.,]+)").ok();
        let total_re = Regex::new(r"Zu Ihren (?:Lasten|Gunsten)\s*:\s*EUR\s*([\d.,]+)").ok();
        ...
        let name_re = Regex::new(r"Wertpapier-Bezeichnung\s*:\s*(.+?)(?:\n|ISIN)").ok();
    ...
    fn parse_dividends(&self, content: &str, ctx: &mut ParseContext) -> Vec<ParsedTransaction> {
        ...
        let date_re = Regex::new(r"Valuta\s*:\s*(\d{2}\.\d{2}\.\d{4})").ok();
        let isin_re = Regex::new(r"ISIN\s*:\s*([A-Z]{2}[A-Z0-9]{10})").ok();
        let shares_re = Regex::new(r"St(?:ü|u)ck\s*:\s*([\d.,]+)").ok();
        let gross_re = Regex::new(r"Brutto\s*:\s*EUR\s*([\d.,]+)").ok();
        let tax_re = Regex::new(r"Kapitalertragsteuer\s*:\s*EUR\s*([\d.,]+)").ok();
        let soli_re = Regex::new(r"Solidarit(?:ä|a)tszuschlag\s*:\s*EUR\s*([\d.,]+)").ok();
        let net_re = Regex::new(r"Zu Ihren Gunsten\s*:\s*EUR\s*([\d.,]+)").ok();
    ...
    fn parse_savings_plan(&self, content: &str, ctx: &mut ParseContext) -> Vec<ParsedTransaction> {
        let mut transactions = Vec::new();

        if !content.contains("Sparplanausführung") && !content.contains("Wertpapiersparplan") {
            return transactions;
        }

        let date_re = Regex::new(r"Ausf(?:ü|u)hrungstag\s*:\s*(\d{2}\.\d{2}\.\d{4})").ok();
        let isin_re = Regex::new(r"ISIN\s*:\s*([A-Z]{2}[A-Z0-9]{10})").ok();
        let shares_re = Regex::new(r"St(?:ü|u)ck\s*:\s*([\d.,]+)").ok();
        let price_re = Regex::new(r"Kurs\s*:\s*([\d.,]+)\s*EUR").ok();
        let amount_re = Regex::new(r"Sparrate\s*:\s*EUR\s*([\d.,]+)").ok();
        ...
        let mut txn = ParsedTransaction {
            ...
            net_amount: 0.0,
            currency: "EUR".to_string(),
            note: Some("Sparplanausführung".to_string()),
            exchange_rate: None,
            forex_currency: None,
        };
...
impl BankParser for ComdirectParser {
    ...
    fn parse(&self, content: &str, ctx: &mut ParseContext) -> Result<Vec<ParsedTransaction>, String> {
        ...
        transactions.extend(self.parse_savings_plan(content, ctx));
