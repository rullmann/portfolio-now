...
use crate::ai::{
    ...
    SoldPosition, YearlyOverview, QuoteProviderStatusSummary,
...
fn load_portfolio_context(base_currency: &str, user_name: Option<String>) -> Result<PortfolioInsightsContext, String> {
    ...
    // Uses fifo::get_cost_basis_by_security_id_converted() which converts each lot individually
...
fn load_provider_status_for_ai(conn: &rusqlite::Connection) -> Option<QuoteProviderStatusSummary> {
    use crate::ai::QuoteSyncInfo;
    ...
    // Query securities with their providers AND latest quote date
    let sql = r#"
        SELECT
            s.id,
            s.name,
    ...
    let rows = match stmt.query([&today_str]) {
        Ok(r) => r,
        Err(_) => return None,
    };
    ...
    for (_id, name, provider, ticker, isin, last_quote_date, days_old_f) in &securities {
        ...
        if provider_upper.is_empty() || provider_upper == "MANUAL" || provider_upper == "GENERIC_HTML_TABLE" {
            ...
            // We assume API keys are missing in this simplified check
            cannot_sync_count += 1;
    ...
    // Always return status so AI knows about quote freshness
...
/// Load fees and taxes summary
fn load_fees_and_taxes(conn: &rusqlite::Connection) -> FeesAndTaxesSummary {
    let current_year = Utc::now().year();

    // Query total fees and taxes, and by year
    let sql = r#"
        SELECT
...
fn load_investment_summary(conn: &rusqlite::Connection) -> InvestmentSummary {
    // Sum of all BUY transactions (amount is the total cost)
    let buy_sql = r#"
    ...
    // Sum of all DEPOSIT transactions to accounts
    let deposit_sql = r#"
        SELECT COALESCE(SUM(amount), 0) FROM pp_txn
    ...
    // Sum of all REMOVAL transactions from accounts
    let removal_sql = r#"
        SELECT COALESCE(SUM(amount), 0) FROM pp_txn
...
/// Load sector/taxonomy allocation
fn load_sector_allocation(conn: &rusqlite::Connection, total_value: f64) -> Vec<SectorAllocation> {
    if total_value <= 0.0 {
        return Vec::new();
    }

    // Get all taxonomies with their classifications and security assignments
    let sql = r#"
        SELECT
    ...
    if let Ok(mut stmt) = conn.prepare(sql) {
        if let Ok(rows) = stmt.query_map([], |row| {
            Ok((
                row.get::<_, String>(0)?,          // taxonomy_name
                row.get::<_, String>(1)?,          // classification_name
                row.get::<_, i64>(2)?,             // net_shares (scaled)
                row.get::<_, Option<i64>>(3)?,     // price (scaled)
                row.get::<_, i32>(4)?,             // weight (percentage * 100)
            ))
        }) {
            for row in rows.flatten() {
                ...
                // Apply the weight from the assignment (weight is 0-10000 for 0-100%)
                let weighted_value = position_value * (weight as f64 / 10000.0);

                let entry = taxonomy_map.entry(tax_name).or_insert_with(Vec::new);
                // Find or create classification entry
...
fn load_portfolio_extremes(conn: &rusqlite::Connection) -> Option<PortfolioExtremes> {
    let current_year = Utc::now().year();
