...
// ============================================================================
// Query Type Approval (Security Feature)
// ============================================================================

use crate::ai::{
    QueryType,
    approve_query_type, get_approved_query_types, revoke_all_approvals,
    execute_transaction_queries, execute_portfolio_value_queries, execute_db_queries,
    TransactionQuery, PortfolioValueQuery, DbQuery,
};

/// Approve a query type for the current session.
///
/// SECURITY: This should only be called after explicit user confirmation.
/// Once approved, queries of this type will be auto-executed for the session.
#[command]
pub fn approve_query_type_for_session(query_type: String) -> Result<(), String> {
    let qt: QueryType = query_type.parse()
        .map_err(|e: String| e)?;
...
///
/// SECURITY: This executes the query and returns results.
/// The query_type parameter determines which executor to use.
#[command]
pub fn execute_pending_query(query_type: String, payload: String) -> Result<Vec<String>, String> {
    ...
    match qt {
        QueryType::TransactionQuery => {
            let query: TransactionQuery = serde_json::from_str(&payload)
                .map_err(|e| format!("Invalid transaction query payload: {}", e))?;
            Ok(execute_transaction_queries(&[query]))
        }
        QueryType::PortfolioValueQuery => {
            let query: PortfolioValueQuery = serde_json::from_str(&payload)
                .map_err(|e| format!("Invalid portfolio value query payload: {}", e))?;
            Ok(execute_portfolio_value_queries(&[query]))
        }
        QueryType::DatabaseQuery => {
            let query: DbQuery = serde_json::from_str(&payload)
                .map_err(|e| format!("Invalid database query payload: {}", e))?;
            Ok(execute_db_queries(&[query]))
        }
    }
}

/// Get all query types that are currently approved for this session.
#[command]
...
/// Revoke all query type approvals (security reset).
///
/// Call this when the user wants to reset permissions or on security events.
#[command]
